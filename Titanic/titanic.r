...
train_sub<-floor(0.8*nrow(train1))
set.seed(123)
...
set.seed(0)#to get same output again and again
rf<-randomForest(Survived~Pclass+Sex+Age+SibSp+Parch+Fare+Embarked,data=train,ntree=10)
rf
train$pred_rf<-predict(rf,train)
table(train$Survived,train$pred_rf)
x<-(533+274)/(533+274+16+68)
test$Survived<-predict(rf,test)
write.csv(test,'rf_submission1.csv')
train2<-train1[-t,]
str(train2)
train2$Survived<-as.factor(train2$Survived)
train2$Pclass<-as.factor(train2$Pclass)
train2$Age<-ifelse(is.na(train2$Age),median(train2$Age,na.rm=TRUE),train2$Age)
train2$p<-predict(rf,train2)
table(train2$p,train2$Survived)